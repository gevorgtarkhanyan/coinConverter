...
extension BaseSegmentControl {
    ...
    func addBottomLIne() {
        ...
        bottomLine.bottomAnchor.constraint(equalTo: self.bottomAnchor, constant: -constant - 1).isActive = true
        bottomLine.leadingAnchor.constraint(equalTo: self.leadingAnchor,constant: constant).isActive = true
        bottomLine.trailingAnchor.constraint(equalTo: self.trailingAnchor, constant: -constant).isActive = true
        bottomLine.heightAnchor.constraint(equalToConstant: 1).isActive = true
    }

  @objc private func changeColors() {
        backgroundColor = .clear
        bottomLine.backgroundColor = darkMode ? .white : UIColor.black.withAlphaComponent(0.85)
    }
}
...
extension BaseSegmentControl {
    public func setSegments(_ segments: [String],_ isSecondSegmentView: Bool = false) {
        ...
        for i in segments.indices {
            ...
            button.changeFontSize(to: 16)
            button.changeFont(to: Font.boldFont)//
            ...
            if i != segments.indices.last {
                ...
                separatorView.topAnchor.constraint(equalTo: button.topAnchor).isActive = true
                separatorView.widthAnchor.constraint(equalTo: button.widthAnchor, multiplier: 1).isActive = true
                separatorView.leftAnchor.constraint(equalTo: button.rightAnchor).isActive = true
                separatorView.bottomAnchor.constraint(equalTo: button.bottomAnchor).isActive = true
                separatorView.heightAnchor.constraint(equalTo: button.heightAnchor, multiplier: 0.1).isActive = true
            }
    ...
    public func setSegmentsWithImage(_ segments: [String]) {
        ...
        for i in segments.indices {
            ...
            label.changeFont(to: Font.semiboldFont)
...
extension BaseSegmentControl {
    ...
    private func addSelectionView() {
        ...
        bottomLine.bringSubviewToFront(self)

        bottomLine.backgroundColor = darkMode ? .endGradient : .startGradient
        bottomLine.rightAnchor.constraint(equalTo: selectedView.rightAnchor).isActive = true
        bottomLine.leftAnchor.constraint(equalTo: selectedView.leftAnchor).isActive = true
        bottomLine.bottomAnchor.constraint(equalTo: selectedView.bottomAnchor).isActive = true
        bottomLine.heightAnchor.constraint(equalToConstant: 5).isActive = true
    ...
    @objc fileprivate func buttonAction(_ sender: UIView) {
        ...
        for button in  self.buttons {
            if let button = button as? SegmentButton {
                button.setTitleColor(darkMode ? .white : UIColor.black.withAlphaComponent(0.85), for: .normal)
            }
        }
        if let button  = sender as? SegmentButton {
            button.setTitleColor(.endGradient, for: .normal)
        }
